...
is
      ...
      then
         ...
         case Drop is
            when ada.Strings.Right =>
               ...
               Target.Data (1 .. Max_Length) := Source (Source'First .. Source'First - 1 + Max_Length);

            when ada.Strings.Left =>
               Target.Current_Length         := Max_Length;
               Target.Data (1 .. Max_Length) := Source (Source'Last - (Max_Length - 1) .. Source'Last);
         ...
         case Drop is
            when ada.Strings.Right =>
               if Llen >= Max_Length then -- only case is Llen = Max_Length
                  ...
                  Result.Data (1 .. Llen)              := Left.Data (1 .. Llen);
                  Result.Data (Llen + 1 .. Max_Length) := Right.Data (1 .. Max_Length - Llen);
               end if;
            ...
            when ada.Strings.Left =>
               if Rlen >= Max_Length then -- only case is Rlen = Max_Length
                  ...
                  Result.Data (1 .. Max_Length - Rlen)              := Left.Data (Llen - (Max_Length - Rlen - 1) .. Llen);
                  Result.Data (Max_Length - Rlen + 1 .. Max_Length) := Right.Data (1 .. Rlen);
               end if;
      ...
      then
         ...
         case Drop is
            ...
            when ada.Strings.Left =>
               if Rlen >= Max_Length
               then
                  Result.Data (1 .. Max_Length) := Right (Right'Last - (Max_Length - 1) .. Right'Last);

               else
                  Result.Data (1 .. Max_Length - Rlen)              := Left.Data (Llen - (Max_Length - Rlen - 1) .. Llen);
                  Result.Data (Max_Length - Rlen + 1 .. Max_Length) := Right;
               end if;
      ...
      then
         ...
         case Drop is
            when ada.Strings.Right =>
               if Llen >= Max_Length
               then
                  Result.Data (1 .. Max_Length) := Left (Left'First .. Left'First + (Max_Length - 1));
            ...
            when ada.Strings.Left =>
               if Rlen >= Max_Length
               then
                  Result.Data (1 .. Max_Length) := Right.Data (Rlen - (Max_Length - 1) .. Rlen);

               else
                  Result.Data (1 .. Max_Length - Rlen)              := Left (Left'Last - (Max_Length - Rlen - 1) .. Left'Last);
                  Result.Data (Max_Length - Rlen + 1 .. Max_Length) := Right.Data (1 .. Rlen);
               end if;
   ...
   begin
      ...
      then
         ...
         case Drop is
            ...
            when ada.Strings.Left =>
               Result.Current_Length             := Max_Length;
               Result.Data (1 .. Max_Length - 1) := Left.Data (2 .. Max_Length);
      ...
      then
         ...
         case Drop is
            when ada.Strings.Right =>
               Result.Current_Length         := Max_Length;
               Result.Data (1)               := Left;
               Result.Data (2 .. Max_Length) := Right.Data (1 .. Max_Length - 1);
               return Result;
   ...
   begin
      ...
      then
         ...
         Result.Data (1 .. From - 1)                 := Source.Data (1 .. From - 1);
         Result.Data (From .. Result.Current_Length) := Source.Data (Through + 1 .. Slen);
         return Result;
      end if;
   end Super_Delete;
   ...
   begin
      ...
      then
         ...
         Source.Data (From .. Source.Current_Length) := Source.Data (Through + 1 .. Slen);
      end if;
   end Super_Delete;
   ...
   is
   begin
      if Index <= Source.Current_Length
      then
         ...
         case Drop is
            ...
            when ada.Strings.Left =>
               ...
               then
                  ...
                  Result.Data (1 .. Max_Length - Npad)              := Source.Data (Count - Max_Length + 1 .. Slen);
                  Result.Data (Max_Length - Npad + 1 .. Max_Length) := [others => Pad];
               end if;
         ...
         case Drop is
            ...
            when ada.Strings.Left =>
               ...
               then
                  ...
                  Temp                                 := Source.Data;
                  Source.Data (1 .. Max_Length - Npad) := Temp (Count - Max_Length + 1 .. Slen);

                  for J in Max_Length - Npad + 1 .. Max_Length
                  loop
                     Source.Data (J) := Pad;
                  end loop;
               end if;
   ...
   begin
      ...
      then
         ...
         case Drop is
            ...
            when ada.Strings.Left =>
               Result.Data (Max_Length - (Alen - 1) .. Max_Length) := Source.Data (Before .. Slen);
      ...
      then
         ...
         begin
            ...
            then
               ...
               Result.Data (Low + By'Length .. Tlen)    := Source.Data (High + 1 .. Slen);
               ...
               case Drop is
                  when ada.Strings.Right =>
                     ...
                     if Droplen > Alen
                     then
                        Result.Data (Low .. Max_Length) := By (By'First .. By'First + Max_Length - Low);
                     else
                        Result.Data (Low .. Low + By'Length - 1)    := By;
                        Result.Data (Low + By'Length .. Max_Length) := Source.Data (High + 1 .. Slen - Droplen);
                     end if;

                  when ada.Strings.Left =>
                     Result.Data (Max_Length - (Alen - 1) .. Max_Length) := Source.Data (High + 1 .. Slen);
      ...
      then
         ...
         case Drop is
            when ada.Strings.Right =>
               ...
               while Indx + Item'Length <= Max_Length + 1
               loop
                  Result.Data (Indx .. Indx + Item'Length - 1) := Item;
                  Indx                                         := Indx + Item'Length;
               end loop;

               Result.Data (Indx .. Max_Length) := Item (Item'First .. Item'First + Max_Length - Indx);

            when ada.Strings.Left =>
               Indx := Max_Length;

               while Indx - Item'Length >= 1
               loop
                  Result.Data (Indx - (Item'Length - 1) .. Indx) := Item;
                  Indx                                           := Indx - Item'Length;
               end loop;

               Result.Data (1 .. Indx) := Item (Item'Last - Indx + 1 .. Item'Last);
   ...
   begin
      ...
      then
         Result.Current_Length    := Count;
         Result.Data (1 .. Count) := Source.Data (Slen - (Count - 1) .. Slen);
         ...
         case Drop is
            when ada.Strings.Right =>
               ...
               then
                  ...
                  Result.Data (1 .. Npad)              := [others => Pad];
                  Result.Data (Npad + 1 .. Max_Length) := Source.Data (1 .. Max_Length - Npad);
               end if;

            when ada.Strings.Left =>
               Result.Data (1 .. Max_Length - Slen)              := [others => Pad];
               Result.Data (Max_Length - Slen + 1 .. Max_Length) := Source.Data (1 .. Slen);
   ...
   begin
      ...
      then
         Source.Current_Length    := Count;
         Source.Data (1 .. Count) := Temp (Slen - (Count - 1) .. Slen);
         ...
         case Drop is
            when ada.Strings.Right =>
               ...
               then
                  ...
                  Source.Data (1 .. Npad)              := [others => Pad];
                  Source.Data (Npad + 1 .. Max_Length) := Temp (1 .. Max_Length - Npad);
               end if;

            when ada.Strings.Left =>
               for J in 1 .. Max_Length - Slen
               loop
                  Source.Data (J) := Pad;
               end loop;

               Source.Data (Max_Length - Slen + 1 .. Max_Length) := Temp (1 .. Slen);
   ...
   begin
      ...
      for J in 1 .. Source.Current_Length
      loop
   ...
   begin
      if Side = Left or else Side = Both
      then
         while First <= Last and then Source.Data (First) = ' '
         loop
            First := First + 1;
         end loop;
      end if;

      if Side = Right or else Side = Both
      then
         while Last >= First and then Source.Data (Last) = ' '
         loop
            Last := Last - 1;
         end loop;
      end if;
   ...
   begin
      ...
      if Side = Left or else Side = Both
      then
         while First <= Last and then Temp (First) = ' '
         loop
            First := First + 1;
         end loop;
      end if;
      ...
      if Side = Right or else Side = Both
      then
         while Last >= First and then Temp (Last) = ' '
         loop
            Last := Last - 1;
         end loop;
      end if;
   ...
   begin
      for First in 1 .. Source.Current_Length
      loop
         if not Is_In (Source.Data (First), Left)
         then
            for Last in reverse First .. Source.Current_Length
            loop
               if not Is_In (Source.Data (Last), Right)
               then
                  Result.Current_Length                    := Last - First + 1;
                  Result.Data (1 .. Result.Current_Length) := Source.Data (First .. Last);
                  return Result;
   ...
   begin
      ...
      then
         ...
         then
            for J in 1 .. Left
            loop
               Result.Data (Pos .. Pos + Rlen - 1) := Right.Data (1 .. Rlen);
               Pos                                 := Pos + Rlen;
            end loop;
         end if;
      end if;
   ...
   begin
      ...
      then
         ...
         case Drop is
            when ada.Strings.Right =>
               Result.Current_Length         := Max_Length;
               Result.Data (1 .. Max_Length) := Source (Source'First .. Source'First - 1 + Max_Length);

            when ada.Strings.Left =>
               Result.Current_Length         := Max_Length;
               Result.Data (1 .. Max_Length) := Source (Source'Last - (Max_Length - 1) .. Source'Last);
